...
int main() {//Eject
	...
	/*Pregunta las opciones, si no entiendes japones a continuacion te las pongo
	                Bienvenido al banco elija una opcion para ejecutar
								...
								3-Cerrar banco
*/
switch(opcion) {//Haremos un switch de opcion para ejecutar segun la opcion que elijio
	case 1:{
		system("CLS");//Borrara todo el historial
	/*cout<<"Yuza-mei o iu: ";//Pedira el nombre de usurio
	cin>>user;//Guardara el nombre de usurio
	*/
	cout<<"Nyuryoku suru atai o nyuryoku shimasu: ";cin>>valor;//Preguntara cuanto quiere ingresar y luego lo pondra en la variable
    cout<<"Kanpekidesu, ima anata wa motte imasu: "<<valor+total<<"\n\n\t1 O nyuryoku shite akushon o jikko shi, 2 o nyuryoku shite jikko o shuryo shimasu: ";cin>>op2;//1-Dira cuanto tiene en total. 2-Dira si quiere realiazr mas ejecuciones
    switch(op2) {//Otro switch
    	case 1:system("CLS");exit(main());break;//En el caso de uno, borra la consola y vuelve
    		case 2:system("CLS");system("exit");return 0;break;//En el caso de dos, borra la consola y termina
    			case 3:system("CLS");system("exit");return 0;break;//En el caso de tres, borra la consola y termina
					default:system("CLS");system("exit");return 0;break;//En el caso de nada, borra la consola y termina
    }
	getch();//Para que no se cierre por que si
	}break;//Caso de la opcion 1
		case 2:{
		system("CLS");//Borrara todo el historial
		cout<<"Hikidashitai kingaku o nyuryoku shite kudasai: ";cin>>valor;//Preguntara cuanto quiere retirar
		if(valor > total) {//Si lo que quiere sacar es mayor a lo que tiene
	...
	getch();//Para que no se cierre por que si
			...
			case 3:system("CLS");system("exit");return 0;break;//Caso de la opcion 3 retorna xd
				default:system("CLS");system("exit");return 0;break;//En el caso de nada, borra la consola y termina
	getch();//Para que no se cierre por que si
}						
	getch();//Para que no se cierre por que si
